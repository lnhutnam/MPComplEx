...
class MPComplEx(TKBCModel):
    ...
    def score(self, x):
        ...
        # Improve solution 1
        rhs = (
            self.beta1 * rhs[:, : self.rank] + self.beta2 * time[:, 4 * self.rank : 5 * self.rank],
            self.beta1 * rhs[:, self.rank :] + self.beta2 * time[:, 5 * self.rank : 6 * self.rank],
        )

        # rel = rel[:, : self.rank], rel[:, self.rank : 2 * self.rank]
        rel = (
            self.gamma_rel * rel[:, : self.rank] + (1 - self.gamma_rel) * rcontext_rel,
            self.gamma_rel * rel[:, self.rank : 2 * self.rank] + (1 - self.gamma_rel) * icontext_rel,
        )
    ...
    def get_queries(self, queries: torch.Tensor):
        ...
        # Improve solution 1
        lhs = (
            self.alpha1 * lhs[:, : self.rank] + self.alpha2 * time[:, 2 * self.rank : 3 * self.rank],
            self.alpha1 * lhs[:, self.rank :]
            + self.alpha2 * time[:, 3 * self.rank : 4 * self.rank],
        )
        ...
        rel = (
            self.gamma_rel * rel[:, : self.rank] + (1 - self.gamma_rel) * rcontext_rel,
            self.gamma_rel * rel[:, self.rank : 2 * self.rank]
            + (1 - self.gamma_rel) * icontext_rel,
        )
