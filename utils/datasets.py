...
class TemporalDataset(object):
    def __init__(self, name: str):
        ...
        # for f in ["invs", "syms", "comps"]:
        #     if os.path.isfile(str(self.root / (f + ".pickle"))):
        #         in_file = open(str(self.root / (f + ".pickle")), "rb")
        #         self.data[f] = pickle.load(in_file)

        # for f in ["ct_invs", "ct_hierarchy", "ct_intersection", "ct_comps"]:
        # # for f in ["ct_invs", "ct_hierarchy", "ct_intersection",]:
        #     if os.path.isfile(str(self.root / (f + ".pickle"))):
        #         in_file = open(str(self.root / (f + ".pickle")), "rb")
        #         self.data[f] = pickle.load(in_file)
        
        for f in ["test_Arrest",]:
            if os.path.isfile(str(self.root / (f + ".pickle"))):
                in_file = open(str(self.root / (f + ".pickle")), "rb")
                self.data[f] = pickle.load(in_file)
        ...
        try:
            ...
            self.time_diffs = torch.from_numpy(
                pickle.load(inp_f)).cuda().float()
            # print("Assume all timestamps are regularly spaced")
            # self.time_diffs = None
            inp_f.close()
        except OSError:
            logging.info("Assume all timestamps are regularly spaced")
            self.time_diffs = None
        ...
        try:
            ...
            logging.info("Not using time intervals and events eval")
            self.events = None
    ...
    def time_eval(
        ...
        sampled_time = (
            (
                torch.rand(time_range.shape[0]) *
                (time_range[:, 4] - time_range[:, 3])
                + time_range[:, 3]
            )
            .round()
            .long()
        ...
        hits_at = {
            z: torch.FloatTensor(
                (list(map(lambda x: torch.mean(
                    (ranks[z] <= x).float()).item(), at)))
            )
            for z in ranks
            if len(ranks[z]) > 0
        }
    ...
    def breakdown_time_eval(
        ...
        sum_reciprocal_rank = {x: torch.sum(
            1.0 / ranks[x]).item() for x in ranks}
